...
/*   Updated: 2025/11/13 12:30:08 by hrami            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_weapon(t_game *game)
{
	...
	while (i < wea->toto_frames)
	{
		wea->frames[i] = mlx_xpm_file_to_image(game->mlx, wea_pathe[i],
				&w_width, &w_height);
		if (!wea->frames[i])
		{
...
int	get_pixel_color(t_img *img, int x, int y)
{
	...
	pixel = img->img_p_data + (y * img->line_len + x * (img->bit_p_pixle / 8));
	return (*(int *)pixel);
}
...
void	draw_weapon(t_game *game)
{
	...
	frame.img_p_data = mlx_get_data_addr(frame.img, &frame.bit_p_pixle,
			&frame.line_len, &frame.endain);
...
//     mlx_put_image_to_window(game->mlx, game->window,
		// wea->frames[wea->current_frame], x, y);
// }
